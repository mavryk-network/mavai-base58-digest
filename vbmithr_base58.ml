...
let of_bytes ?alphabet c s = `Base58 (raw_encode ?alphabet (s ^ checksum c s))
...
let to_bytes ?alphabet c (`Base58 s) =
  ...
  if msg_hash <> checksum c msg then None else Some msg
...
module Mavryk = struct
  ...
  let of_string c str =
    match of_string c str with None -> None | Some b58 -> of_base58 c b58
...
module Bitcoin = struct
  ...
  let t_of_bytes s =
    ...
    | '\x04' :: '\x88' :: '\xAD' :: '\xE4' :: _ ->
        (BIP32_priv, String.sub s 4 (len - 4))
    | '\x04' :: '\x88' :: '\xB2' :: '\x1E' :: _ ->
        (BIP32_pub, String.sub s 4 (len - 4))
    | '\x04' :: '\x35' :: '\x83' :: '\x94' :: _ ->
